...
@property (nonatomic, strong) UITapGestureRecognizer *tapRecognizer;   //控制手势
...
- (void)fullScreen:(UIDeviceOrientation)orientation {
    ...
    if (orientation == UIDeviceOrientationLandscapeLeft
        ...
        CGFloat height = self.superview.frame.size.width;
        CGFloat width = self.superview.frame.size.height;
        ...
        [UIView animateWithDuration:0.3f animations:^{
            [self setTransform:tranform];
            [self resetFrame:self.superPlayView.bounds];
...
- (void)tapRecognizer:(UITapGestureRecognizer *)tap {
    [self.playerControl showOrHidenBar];
}
...
- (void)setSuperPlayView:(UIView *)superPlayView {
    ...
    self.frame = superPlayView.bounds;
    self.playerControl.frame = self.bounds;
